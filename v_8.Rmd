...
gum_clean <- gum %>%
     select(pdbase, attachbase, pd1year, attach1year) %>%
     na.omit() 
...
# S Calculate the correlation matrix
 ...
 #matrix shows correlation, appropriate to use change variable. 
```
...
model_attach <- lm(attach ~ group1 +group3 +group4 +group5 +gender, data = gum_with_groups)
summary(model_attach)
confint(model_attach, level = 0.95)

#output final model with SjPlot package
tab_model(model_attach,
          title = "Linear Regression Output of Treatment Groups, Gender on Attachment Loss",
          show.stat = TRUE,
